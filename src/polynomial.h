...
namespace iRRAM{
  ...
  public:
    ...
    Polynomial(const std::string& s, const std::array<char,d>& variables, const REAL& radius){
      this->radius = radius;
      ...
      for(int i=0; i<substr.size(); i++){
        coeffs[i] = Polynomial<d-1,REAL>(substr[i], vars, radius);
      }
    }
  ...
  public:
    ...
    Polynomial(const std::string& s, const std::array<char,0>& variables, const REAL& radius){
      std::stringstream ss(s);
      std::string token;
      while (std::getline(ss, token, '+')) {
        this->value += token;
...
// scalar multiplication
  ...
  Polynomial<d,T> operator*(const T& lhs, const Polynomial<d,T>& rhs){
    ...
    return Polynomial<d,T>(coeffs, rhs.get_radius());
  }
  ...
  MVPowerseries<d,m,n,T> P2M(const std::array<std::array<std::string, n>,m>& M, const std::array<char,d>& variables){
    ...
    for(int i=0; i<n; i++){
      for(int j=0; j<m; j++){
        std::shared_ptr<Powerseries<d,T>> p(new Polynomial<d,T>(M[i][j], variables, 1)); 
        Mptr[i][j] = p;
      }
    }
