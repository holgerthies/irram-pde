...
namespace iRRAM{
  ...
  private:
    ...
    double get_factor(int n, int m) const{
      return FactorCache::get_factor(n,m);
    }
    double get_factor(const std::array<unsigned int,d>& n,const std::array<unsigned int,d>& m) const{
      REAL ans=1;
      for(int i=0; i<d; i++)
        ans *= FactorCache::get_factor(n[i], m[i]);
      return ans;
    }
    double get_bound_complex(const int deg, const std::array<T,d>& x) const{
      REAL r = get_radius();
  ...
  public:
    ...
    T sum(const vector<T,d>& x) const {
      ...
      double error_factor = power(xmax/radius,d);
