...
namespace iRRAM{
  ...
  template<unsigned int n, class T> class Cinfinity;

  template<unsigned int n, class T>
  using CinfinityPtr = std::shared_ptr<Cinfinity<n,T>>;

  template<unsigned int n, class T>
  class Cinfinity : public std::enable_shared_from_this<Cinfinity<n,T>>{
  ...
  public:
    ...
    virtual Cinfinity<n,T> add(const CinfinityPtr<n,T>& f) const;
    virtual Cinfinity<n,T> subtract(const CinfinityPtr<n,T>& f) const;
    virtual Cinfinity<n,T> multiply(const CinfinityPtr<n,T>& f) const;
    virtual Cinfinity<n,T> multiply(const T& m) const;
...
// leibniz rule
  ...
  Cinfinity<n,T> Cinfinity<n,T>::multiply(const CinfinityPtr<n,T>& f) const{
    auto self = this->shared_from_this();
    return Cinfinity<n,T>([self,f] (auto index, auto x) {return product_rule<n>(*self, *f, index, x);}, [self,f](auto index, auto x, auto eps) {return product_rule_bound<n>(*self,*f,index,x,eps);} );
  }
