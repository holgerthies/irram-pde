...
void compute(){
  ...
  std::shared_ptr<Cinfinity<2,REAL>> sinex = std::make_shared<Cinfinity<2,REAL>>(fx, [] (auto index, auto x, auto eps) { cout << "sinex called " << index[0] << " " << index[1] << std::endl; if(index[1] > 0) return 0; else return 1;} );
  std::shared_ptr<Cinfinity<2,REAL>> siney = std::make_shared<Cinfinity<2,REAL>>(fy, [] (auto index, auto x, auto eps) {cout << "cosinex called " << index[0] << " " << index[1] << std::endl; if(index[0] > 0) return 0; else return 1;} );
  auto cosiney = std::make_shared<Cinfinity<2,REAL>>(siney->derive({0,1}));
  auto sxcy = sinex*cosiney;
  auto x2 = monomial<REAL>(2,5);
  cout << x2({1.0}) << std::endl;
  auto x = x2.derive({1});
  cout << x({1.0}) << std::endl;
  ...
  PS_ptr<1,REAL> s = std::make_shared<Powerseries<1,REAL>>(sine,center1d,REAL(1));
  auto cosine = std::make_shared<Cinfinity<1,REAL>>(sine->derive({1}));
  PS_ptr<1,REAL> c = std::make_shared<Powerseries<1,REAL>>(cosine,center1d,REAL(1));
  ...
  MVPowerseries<1,2,1,REAL> v1d({{{s},{c}}});
  std::array<MVPowerseries<1,2,2,REAL>,1> farr1d = {{fun1d}};
  auto sol = Pde_local_solution<2,1,REAL>(farr1d, v1d, center1d);
  print(sol({1}));
  //print(fun1d({0.2}));
